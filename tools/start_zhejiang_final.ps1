...
$logPath = Join-Path $projectRoot "logs\lmpc_zhejiang_final.csv"
$outPath = Join-Path $projectRoot "logs\lmpc_zhejiang_final.out.log"
$errPath = Join-Path $projectRoot "logs\lmpc_zhejiang_final.err.log"
...
$arguments = @(
    ...
    "--no-autotune",
    "--resume-tuning",
    "configs\final\zhejiang_final.json",
    "--mpc-model",
...
Write-Output "Zhejiang final launcher PID: $($process.Id)"
